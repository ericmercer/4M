...
(test-expr '(truncate "hello" 10) "hello")
(test-expr '(truncate "hello" 2) "he")
(test-expr '(truncate "hello" 0) "")
(test-expr '(truncate "hello" -1) 'error)
(test-expr '(truncate "hello" 3.2) "hel")
(test-expr '(truncate "hello" "world") 'error)
...
;Test cases added for deref.  Bug: (tuple (@ "A")) -> "A", but should give error or be stuck
#;(test-expr '(@ "A") 'error)
#;(test-expr '(tuple (@ "A")) '(const-tuple error))
(test--> expr-reductions
         (term (mt mt (@ "A") ret))
         (term (mt mt "A" (@ * -> ret))))
...
(test--> acmd-reductions
         (term ((mt [1 -> 2]) mt ((alloc x))))
         (term (((mt [1 -> 2]) [2 -> 0]) (mt [x -> (addr 2)]) ())))
